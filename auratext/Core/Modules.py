...
def add_pdf_tab(self, tab, path, tab_name):
    if hasattr(self,"webView")==False or self.webView==None:
    ...
    # tab.addTab(webView,tab_name)

    self.spliiter.addWidget(self.webView)
    # if hasattr(self,"text_editor") and self.text_editor!=None:
    # self.spliiter.setStretchFactor(0,1)
    # self.spliiter.setStretchFactor(1, 1)
    # self.spliiter.
    sp = self.tab_widget.sizePolicy()
    sp.setHorizontalStretch(6)
    self.tab_widget.setSizePolicy(sp)

def add_pdf_dock(self,path,tab):
    # a = 1
    try:
        filename = os.path.basename(path)
        pdf_dock = QDockWidget(filename, self)
        pdf_dock.resize(QSize(800,1200))
        # pdf_dock.setGeometry(100,100,800,1200)
        # pdf_dock.setMinimumWidth(600)
        ...
        webView.setUrl(QUrl("file:///" + path.replace('\\', '/')))
        pdf_dock.setWidget(webView)
        pdf_dock.setAllowedAreas(Qt.DockWidgetArea.AllDockWidgetAreas)
        # if self.exp_dock!=None:
        #     self.tabifyDockWidget(self.exp_dock,pdf_dock)
        # else:
